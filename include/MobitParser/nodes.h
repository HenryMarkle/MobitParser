...
namespace mp {
    ...
    /// TODO: implement this
    inline int operator_precedence(token_type) {
        return 0;
    }
    ...
    public:
        ...
        BinOp(binary_operation op_, std::unique_ptr<Node>&& left_, std::unique_ptr<Node>&& right_);
    };
    ...
    inline std::ostream& operator<<(std::ostream& stream, unary_operation op) {
        switch (op) {
            case unary_operation::logical_negation: stream << "not"; break;
            case unary_operation::mathema_negation: stream << '-'; break;
            case unary_operation::mathema_affirmation: stream << '+'; break;
        }
    ...
    inline std::ostream& operator<<(std::ostream& stream, binary_operation op) {
        switch (op) {
            case binary_operation::addition: stream << "+"; break;
            case binary_operation::subtraction: stream << "-"; break;
            case binary_operation::multiplication: stream << "*"; break;
            case binary_operation::division: stream << "/"; break;
            case binary_operation::member_access: stream << '.'; break;
            case binary_operation::and_: stream << "and"; break;
            case binary_operation::or_: stream << "or"; break;
        }
    ...
    std::unique_ptr<Node> parse(const std::string&);
};